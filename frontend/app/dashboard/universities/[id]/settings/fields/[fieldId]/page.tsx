...
const FIELD_TYPES: FieldType[] = [
  ...
  'BOOLEAN', 'SELECT', 'MULTI_SELECT', 'FILE', 'IMAGE', 'URL', 'EMAIL', 'JSON'
];

const EDIT_METHODS: EditMethod[] = [
  'INPUT', 'WYSIWYG', 'MARKDOWN', 'CODE', 'DATE_PICKER', 'DATETIME_PICKER',
  'FILE_UPLOAD', 'IMAGE_UPLOAD', 'SELECT_DROPDOWN', 'CHECKBOX', 'RADIO', 'SLIDER', 'COLOR_PICKER'
];

export default function EditFieldConfigPage() {
  ...
  const { data: fieldConfig, isLoading } = useQuery<UniversityFieldConfig>({
    queryKey: ['university-field-config', fieldId],
    queryFn: async () => {
      const response = await apiClient.get(`/api/university-field-config/${fieldId}`);
      return response.data;
    },
    enabled: !isNew && !!fieldId,
  });
  ...
  const {
    ...
    reset,
    watch,
  } = useForm<UniversityFieldConfigRequest>({
    defaultValues: {
  ...
  useEffect(() => {
    if (fieldConfig && !isNew) {
      reset({
        universityId: fieldConfig.universityId,
        ...
        editMethod: fieldConfig.editMethod,
        editOptions: fieldConfig.editOptions || '',
        validationRules: fieldConfig.validationRules || '',
        displayConfig: fieldConfig.displayConfig || '',
        description: fieldConfig.description || '',
      });
    }
  }, [fieldConfig, isNew, reset]);

  const mutation = useMutation({
    mutationFn: async (data: UniversityFieldConfigRequest) => {
      if (isNew) {
        const response = await apiClient.post('/api/university-field-config', data);
        return response.data;
      } else {
        const response = await apiClient.put(`/api/university-field-config/${fieldId}`, data);
        return response.data;
      }
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['university-field-configs', universityId] });
      router.push(`/dashboard/universities/${universityId}/settings?tab=fields`);
    },
    onError: (err: any) => {
  ...
  const onSubmit = async (data: UniversityFieldConfigRequest) => {
    setLoading(true);
    setError(null);
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-20 items-center">
            <div className="flex items-center space-x-4">
              <button
                onClick={() => router.push(`/dashboard/universities/${universityId}/settings?tab=fields`)}
              ...
              <div>
                <h1 className="text-2xl font-bold gradient-text bg-clip-text">
                  {isNew ? '新規フィールド設定' : 'フィールド設定編集'}
                </h1>
                <p className="text-xs text-slate-500 font-medium">Field Configuration</p>
              </div>
            </div>
          </div>
        </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label htmlFor="fieldKey" className="block text-sm font-bold text-slate-700">
                    <span>フィールドキー</span>
                    <span className="text-red-500">*</span>
                  </label>
                  <input
                    {...register('fieldKey', { required: 'フィールドキーは必須です' })}
                    type="text"
                  ...
                  />
                  {errors.fieldKey && (
                    <p className="text-sm text-red-600">{errors.fieldKey.message}</p>
                  )}
                </div>

                <div className="space-y-2">
                  <label htmlFor="fieldName" className="block text-sm font-bold text-slate-700">
                    <span>フィールド名</span>
                    <span className="text-red-500">*</span>
                  </label>
                  <input
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label htmlFor="fieldType" className="block text-sm font-bold text-slate-700">
                    <span>フィールドタイプ</span>
                    <span className="text-red-500">*</span>
                  </label>
                  <select
                ...
                <div className="space-y-2">
                  <label htmlFor="editMethod" className="block text-sm font-bold text-slate-700">
                    <span>編集方法</span>
                    <span className="text-red-500">*</span>
                  </label>
                  <select
                    {...register('editMethod', { required: true })}
                    className="block w-full border-2 border-slate-200 rounded-xl shadow-sm py-3.5 px-5 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all bg-white/90 hover:bg-white text-slate-800 font-medium"
                  >
                    {EDIT_METHODS.map((method) => (
              ...
              <div className="space-y-2">
                <label htmlFor="defaultValue" className="block text-sm font-bold text-slate-700">
                  デフォルト値
                </label>
                <input
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="flex items-center space-x-2">
                  ...
                  />
                  <label htmlFor="required" className="text-sm font-bold text-slate-700">
                    必須
                  </label>
                </div>
              ...
              <div className="space-y-2">
                <label htmlFor="editOptions" className="block text-sm font-bold text-slate-700">
                  編集オプション (JSON形式)
                </label>
                <textarea
                  {...register('editOptions')}
                  rows={3}
                  ...
                  placeholder='{"options": ["option1", "option2"]}'
                />
              </div>

              <div className="space-y-2">
                <label htmlFor="validationRules" className="block text-sm font-bold text-slate-700">
                  バリデーションルール (JSON形式)
                </label>
                <textarea
              ...
              <div className="space-y-2">
                <label htmlFor="displayConfig" className="block text-sm font-bold text-slate-700">
                  表示設定 (JSON形式)
                </label>
                <textarea
                  ...
                  rows={3}
                  className="block w-full border-2 border-slate-200 rounded-xl shadow-sm py-3.5 px-5 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all bg-white/90 hover:bg-white resize-none text-slate-800 font-medium font-mono text-sm"
                  placeholder='{"width": "100%", "position": "top"}'
                />
              </div>
